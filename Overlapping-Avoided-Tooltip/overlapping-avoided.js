...
function setEdgePosition() {
  for (i = 0; i < markerList.length; i++) {
    ...
    if (!isEdge) {
      if (marker.x < -dragOffset[0] && tooltip.x > -dragOffset[0] - width) {
        tooltip.x = -dragOffset[0] - width;
      } else if (marker.x > window.innerWidth - dragOffset[0] && tooltip.x < window.innerWidth - dragOffset[0]) {
        tooltip.x = window.innerWidth - dragOffset[0];
      }

      if (marker.y < -dragOffset[1] && tooltip.y > -dragOffset[1] - height) {
        tooltip.y = -dragOffset[1] - height;
      } else if (marker.y > window.innerHeight - dragOffset[1] && tooltip.y < window.innerHeight - dragOffset[1]) {
        tooltip.y = window.innerHeight - dragOffset[1];
      }
    }
